...
export default memo(
  function PriceChart({ auction }: { auction: TemplateV1 }) {
  ...
  (prevProps, nextProps) => {
    if (
      prevProps.auction.contributions.length === nextProps.auction.contributions.length &&
      prevProps.auction.totalRaised[0].amount === nextProps.auction.totalRaised[0].amount
    ) {
      return true;
    }
    return false;
