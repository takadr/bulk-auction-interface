...
export default function PriceChart({sale}: {sale: Sale}) {
    ...
    const options = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: `1 ${sale.tokenSymbol.toUpperCase()} = ${getTokenPriceAgainstETHWithMinPrice(sale.minRaisedAmount, sale.allocatedAmount, sale.totalRaised, sale.tokenDecimals).toFixed(8)} ETH`,
    ...
    useEffect(() => {
        ...
        newData.datasets[1].data = sale.contributions.map(contribution => {
            return { 
                ...
                y: getTokenPriceAgainstETHWithMinPrice(sale.minRaisedAmount, sale.allocatedAmount, contribution.totalRaised, sale.tokenDecimals).toNumber()
            }
        });
        // Price at the sale start
        newData.datasets[1].data.unshift({x: sale.startingAt * 1000, y: getMinTokenPriceAgainstETH(sale.minRaisedAmount, sale.allocatedAmount, sale.tokenDecimals).toNumber()});
        // Price now
        const now = new Date().getTime();
        newData.datasets[1].data.push({x: now > sale.closingAt * 1000 ? sale.closingAt * 1000 : now, y: getTokenPriceAgainstETHWithMinPrice(sale.minRaisedAmount, sale.allocatedAmount, sale.totalRaised, sale.tokenDecimals).toNumber()});
